...
// Shopify supports each version for ~12 months. 2025-01 is already end-of-life
// (publicApiVersions on this store lists 2025-10, 2026-01, 2026-04, 2026-07 as
// supported), so this is env-overridable to avoid another silent expiry.
export const API_VERSION = process.env.SHOPIFY_API_VERSION || '2026-04';
...
// ---------------------------------------------------------------- auth
//
// Dev Dashboard apps (the replacement for legacy custom apps, which Shopify
// stopped allowing anyone to create on 2026-01-01) have NO permanent shpat_
// token to copy out of the admin. You exchange the app's client id + secret for
// an access token that lives for 24 hours, and mint a new one when it expires.
//
// Legacy custom apps still work: pass `token` directly and this is skipped.

const TOKEN_ENDPOINT = (store) => `https://${store}/admin/oauth/access_token`;
// Refresh a little early so a request can't start with a token that expires mid-flight.
const TOKEN_SAFETY_MARGIN_MS = 5 * 60 * 1000;

// Module-scoped so a warm serverless instance reuses the token instead of
// minting one per webhook. Keyed by store in case that ever becomes plural.
...
/** Mint (or reuse) an access token via the client credentials grant. */
export async function getAccessToken({ store, clientId, clientSecret, fetchImpl = fetch, now = () => Date.now() }) {
  const cached = tokenCache.get(store);
  if (cached && cached.expiresAt > now()) return cached.token;

  const res = await fetchImpl(TOKEN_ENDPOINT(store), {
    method: 'POST',
  ...
  if (!res.ok) {
    ...
    throw new Error(
      ...
      `Check SHOPIFY_CLIENT_ID/SECRET and that the app is installed on ${store}. ${body.slice(0, 200)}`
    );
  }
  ...
  tokenCache.set(store, {
    ...
    // expires_in is seconds (86399 = 24h).
    expiresAt: now() + (Number(data.expires_in) || 86399) * 1000 - TOKEN_SAFETY_MARGIN_MS,
    scope: data.scope,
  });
  return data.access_token;
}

/** A static token wins; otherwise mint one from client credentials. */
async function resolveToken(creds) {
  if (creds.token) return creds.token;
  if (!creds.clientId || !creds.clientSecret) {
    throw new Error('Need either token, or clientId + clientSecret, to call Shopify');
  }
  return getAccessToken(creds);
}

export async function shopifyGraphQL(query, variables, creds = {}) {
  const { store, fetchImpl = fetch } = creds;
  let token = await resolveToken(creds);
  ...
  for (let attempt = 0; attempt < 5; attempt++) {
    ...
    // A minted token can be revoked or expire early; re-mint once before giving up.
    if (res.status === 401 && !creds.token && !retriedAuth) {
      retriedAuth = true;
      tokenCache.delete(store);
      token = await resolveToken(creds);
      continue;
    }
